...
if command -v python3 >/dev/null 2>&1; then PY=python3
  ...
  # 兜底：按环境变量探测，避免硬编码本机绝对路径
  PY_TRY="${USERPROFILE:-$HOME}/.workbuddy/binaries/python/versions/3.13.12/python.exe"
  if [ -x "$PY_TRY" ]; then PY="$PY_TRY"
  else echo "未找到 python，请安装 python3/python 或把解释器路径加入 PATH"; exit 1; fi
fi
